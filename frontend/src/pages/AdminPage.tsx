  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await adminFetch(`/api/admin/users/${user.id}`, {
        method: "PUT",
        body: JSON.stringify({ email, role, plan_status: planStatus, page_count: count, first_name: firstName, last_name: lastName, phone, company_name: company }),
      });
              ...
              {isLoading ? (
                ...
                users.map((u) => (
                  <tr key={u.id} className="border-b border-border/30 last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => setDetailUser(u)} title="Detalhes" className="p-1.5 rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all"><Eye className="w-4 h-4" /></button>
                        ...
                        <button onClick={() => setHistoryUser(u)} title="Historico de uso" className="p-1.5 rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all"><History className="w-4 h-4" /></button>
