    ...
    const Login = ({ onLogin }) => {
      ...
      const decodeAndNavigate = (token) => {
        try {
          ...
          // --- NOVO: VERIFICAÇÃO DE ATIVIDADE DA CONTA NO FRONTEND ---
          if (!decodedToken.is_active) {
            setErro('Sua conta está inativa. Entre em contato com o suporte para ativá-la.');
            localStorage.removeItem('jwt_token'); // Remove token para evitar loops e garantir que App.js veja como não logado
            onLogin(); // Atualiza o estado de login no App.js para 'false'
            navigate('/login', { replace: true }); // Redireciona de volta para o login
            return; // Interrompe a navegação
          }

          localStorage.setItem('jwt_token', token); // Armazena o token
          onLogin(); // Chama a função do App.js para atualizar o estado de login
      ...
      useEffect(() => {
          ...
          if (token) {
              decodeAndNavigate(token); // Usa a função auxiliar para Google Login
          }
      }, [onLogin, navigate]);
      ...
      const handleLogin = async (e) => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            decodeAndNavigate(data.access_token); // Usa a função auxiliar para login tradicional
          } else {
