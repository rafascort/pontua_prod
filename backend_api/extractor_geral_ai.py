...
class ExtractorGeralAI:
    ...
    def process_document_batch_fast(self, pdf_path, pages_with_periods):
        ...
        if not gcs_documents: raise ValueError("Nenhuma página foi carregada.")
...
def process_pdf_task(pdf_path, pages_with_periods_json, model_type, user_id):
    ...
    try:
        ...
        final_df = full_final_df.drop_duplicates(subset=['Dia_dt']).set_index('Dia_dt')
        final_df = final_df.reindex(overall_date_range)
