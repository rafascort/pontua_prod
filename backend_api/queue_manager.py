...
def extract_periods():
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as tmp_file:
            file.save(tmp_file.name); pdf_path = tmp_file.name
        ...
        if pdf_path and os.path.exists(pdf_path):
            ...
            except Exception as unlink_e: print(f"Erro ao remover {pdf_path}: {unlink_e}")
    ...
    try:
        ...
        if pages:
            ...
            for part in parts:
                part = part.strip()
                if '-' in part:
                    try: start, end = map(int, part.split('-')); page_list.extend(range(start, end + 1))
...
def get_progress(task_id):
    ...
    status_rq = job.get_status(); progress_data = job.meta.copy()
    if status_rq == 'finished':
        internal_status = progress_data.get('status', 'completed'); progress_data['status'] = internal_status
        if internal_status == 'completed': progress_data['result'] = job.result;
        elif internal_status == 'error': progress_data['error'] = job.meta.get('error', 'Erro interno.')
    elif status_rq == 'queued': progress_data['status'] = 'queued'; progress_data.setdefault('message', 'Na fila...')
    elif status_rq == 'started': progress_data['status'] = 'processing'; progress_data.setdefault('message', 'Processando...')
    elif status_rq == 'failed':
        progress_data['status'] = 'error'; exc_info = job.exc_info
        if exc_info: progress_data['error'] = exc_info.strip().split('\n')[-1]
        else: progress_data['error'] = job.meta.get('error', 'Falhou.')
    else: progress_data['status'] = status_rq; progress_data.setdefault('message', f'Status: {status_rq}')
    progress_data.setdefault('current_step', 0); progress_data.setdefault('total_steps', 1)
    if progress_data['total_steps'] == 0: progress_data['total_steps'] = 1
...
def download_result(task_id):
    ...
    for q in QUEUES.values():
        try: job = q.fetch_job(task_id);
        except Exception as e: print(f"Erro buscar job {task_id} fila {q.name}: {e}"); continue
        if job: break
    if not job: return jsonify({'error': 'Tarefa não encontrada/expirada'}), 404
    claims = get_jwt(); job_user_id = job.meta.get('user_id')
    ...
    def remove_file_after_download(path_to_remove):
        time.sleep(10)
        try: os.unlink(path_to_remove); print(f"Ficheiro {path_to_remove} removido (job {task_id}).")
        except Exception as e: print(f"Erro ao remover {path_to_remove} (job {task_id}): {e}")
    cleanup_thread = threading.Thread(target=remove_file_after_download, args=(file_path,)); cleanup_thread.daemon = True; cleanup_thread.start()
    try: return send_file(file_path, mimetype=mimetype, as_attachment=True, download_name=filename)
    except Exception as send_e: print(f"Erro ao enviar {file_path} (job {task_id}): {send_e}"); return jsonify({'error': 'Erro ao enviar ficheiro.'}), 500

@app.route('/api/health', methods=['GET'])
def health_check():
    redis_status = "Unknown"; db_status = "Unknown"
    try: redis_conn.ping(); redis_status = "OK"
    except Exception as e: redis_status = f"ERROR: {str(e)}"
